...
class DatasetExporter:
    ...
    def __init__(self, read_img_fn=None, read_target_fn=None, img_id_fn=None,
        ...
        Args:
            ...
            blend_alpha (float): alpha used to blend a mask target
        ...
        try:
            ImageFont.truetype(font="DejaVuSans.ttf", size=text_size)
            self.font_path = "DejaVuSans.ttf"
        except OSError:
            font_path = Path(__file__).parent() / "fonts" / "DejaVuSans.ttf"
            ImageFont.truetype(font=font_path.as_posix(), size=text_size)
            self.font_path = font_path.as_posix()

    def _get_font(self):
        return ImageFont.truetype(font=self.font_path, size=self.text_size)
    ...
    def _render_datapoint(self, f, t=None):
        ...
        write_text(img, image_id, (1, img.size[1]),
                   color=self.text_color, font=self._get_font())
        return img
    ...
    def _render_target(self, img, target, scale=1.0):
        ...
        def _render_points_with_label(target):
            ...
            write_obj_label(img, pos, label=target[1], font=self._get_font())

        if isinstance(target, str):
            write_text(img, target, (1, 1), color=self.text_color, font=self._get_font())
            return img
        ...
        if self.n_workers == 1:
            with get_tqdm(total=n_images) as bar:
                for c in range(0, n_images, max_counter):
                    self._worker_task(c, output, size, filename_prefix, img_files, targets, max_counter, n_rows, bar)
        else:
            partial_worker_task = partial(self._worker_task,
                                          output=output,
                                          size=size,
                                          ...
                                          n_rows=n_rows)
            with Parallel(n_jobs=self.n_workers) as parallel:
                parallel(delayed(partial_worker_task)(c) for c in range(0, n_images, max_counter))

    def _worker_task(self, c, output, size, filename_prefix, img_files, targets, max_counter, n_rows, bar=None):
